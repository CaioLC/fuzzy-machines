...
def test_constant_call():
    ...
    cons_func = Constant(0, 10)
    assert cons_func(0, .45) == .45
    assert cons_func([0,1,2,3], .45).all() == np.array([.45,.45,.45,.45]).all()
    assert cons_func(-1, .45) == 0
    cons_func = Constant(-10, 10)
    assert cons_func([-11, -10, 2, 3, 20, 20], .45).all() == np.array([0, .45, .45, .45, 0, 0]).all()

    with pytest.raises(ValueError):
        assert cons_func([-1, 2, 3, 20, 20], -.3).all() == np.array([0, -.3, -0.3, 0, 0]).all()
...
def test_constant_area():
    ...
    cons = Constant(0, 10)
    assert cons.area(.5) == 5
    cons = Constant(-10, 10)
    assert cons.area(.5) == 10
    cons = Constant('-inf', 'inf')
    assert cons.area(.5) == float('inf')
...
def test_linear_call():
    lin = Linear(0, 1)
    assert lin(0) == 0
    assert lin([0,.4,.6,.8, 1]).all() == np.array([0,.4,.6,.8, 1]).all()
    assert lin(-1) == 0
    assert lin([-1, .4,.6, 20, 20]).all() == np.array([0, .4,.6, 0, 0]).all()

    lin = Linear(0, 10)
    assert lin(0) == 0
    assert lin([0, 4, 6, 8, 10]).all() == np.array([0,.4,.6,.8, 1]).all()
    assert lin(-1) == 0
    assert lin([-1, .4, 6, 20, 20]).all() == np.array([0, .04,.6, 0, 0]).all()

    lin = Linear(10, 0)
    assert lin(0) == 1
    assert lin([0, 4, 6, 8, 10]).all() == np.array([1, .8, .6, .4, 0]).all()
    assert lin(-1) == 0
    assert lin([-1, .4, 6, 20, 20]).all() == np.array([0, .96,.4, 0, 0]).all()
